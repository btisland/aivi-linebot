...
## Acceptance Criteria
- [x] `src/app.py` 已建立（Flask 應用程式入口）
- [x] 實作 `POST /webhook` endpoint
- [x] 實作 LINE 簽章驗證（使用 LINE_CHANNEL_SECRET）
- [x] 實作事件類型解析（MessageEvent）
- [x] 實作訊息類型檢查（TextMessageContent）
- [x] 建立 LineBotApi 與 WebhookHandler 實例
- [x] 環境變數載入機制（LINE_CHANNEL_SECRET, LINE_CHANNEL_ACCESS_TOKEN）
- [x] 錯誤處理：簽章驗證失敗、無效的請求
- [x] 本機開發模式：可使用 ngrok 測試
...
## Definition of Done
- [x] `src/app.py` 已建立
- [x] `/webhook` endpoint 可接收 POST 請求
- [x] 簽章驗證邏輯正確
- [x] 環境變數正確載入
- [x] 本機啟動服務無錯誤
- [ ] 使用 ngrok + LINE 實測可收到事件（需要真實 credentials，留待部署後測試）
- [x] 程式碼包含完整的繁體中文註解
- [x] 錯誤處理邏輯完整
