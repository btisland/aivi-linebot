...
## Acceptance Criteria
- [x] `src/scrapers/aivi_scraper.py` 已建立
- [x] 實作 `scrape_aivi_news(max_articles: int = 5)` 非同步函式
- [x] 實作 `parse_articles(html: str, max_articles: int = 5)` 解析函式
- [x] 使用 `httpx` 進行 HTTP 請求（timeout 5 秒，最多重試 2 次）
- [x] 使用 `selectolax` 解析 HTML（選擇器：`h2.archive__item-title > a`）
- [x] 處理相對路徑（加上 `https://www.aivi.fyi` 前綴）
- [x] 錯誤處理：網路錯誤、解析錯誤、找不到文章
- [x] 回傳格式：`List[Dict[str, str]]`，每個 dict 包含 `title` 和 `url`
...
## Definition of Done
- [x] `src/scrapers/aivi_scraper.py` 已實作
- [x] 所有核心函式都有繁體中文 docstring
- [x] 錯誤處理邏輯完整
- [x] 程式碼符合 PEP 8 規範
- [x] 本機測試：可手動執行並取得文章清單
- [x] 日誌記錄完整（使用 Python logging 模組）
